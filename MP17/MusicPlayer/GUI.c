...
void mainGUI() { // 메인 화면
	...
	gotoxy(x + 19, y + 10); scanf_s("%d", &main_input, sizeof(main_input));
}
...
void playmodeGUI() { // 재생 모드 선택
	...
	gotoxy(x + 19, y + 11); scanf_s("%d", &play_input, sizeof(play_input));
}
...
void Question_Continue() { //다음 곡 재생할껀지 물음
	...
	gotoxy(x + 10, y + 10); printf("Input : ");
	gotoxy(x + 19, y + 10); scanf_s("%d", &play_continue_input, sizeof(play_continue_input));
}