...
//				[랜덤모드] - 2번(곡 중복불가 랜덤모드)
					...
					for (int i = 0; i < line_number; i++) {
						...
						else if (play_no_overlap_continue_input == 2) return 0;
					}
					File_close4();
				}
			}
...
//		[메인화면] - 3번(개발자전용기능)
		else if (main_input == 51) {
			int i;
			dev_musiclist_line_read(); //txt파일 라인 수 카운트
			dev_music_search();
			dev_music_play();
			i = first_music_play_count + 1;
			for (i; i < line_number - 1; i++) {
				Question_Continue(); // 다음곡 재생할지 질문
				if (play_continue_input == 1) {
					dev_continue_music_play(); // Yes선택시 다음 링크 재생
				}
				else return 1; // No 선택시 프로그램 종료
			}
			dev_file_close(); // 재생 다했을시 파일닫기 및 특정변수 초기화
		}
