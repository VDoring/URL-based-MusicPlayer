...
int dev_important_music_search(char link[]) { // -- 확인
	...
	*/
	...
	while (1)
	{
		...
		//printf("%s\n", ptr); //<테스트코드> 동작확인
		strcpy(spacial_text, ptr); //--여부 확인위해 special로 문자열을 복사한다
...
void dev_music_search() { // 전체 링크를 탐색해 --가 붙은 링크를 찾음
	...
	for (int i = 0; i < line_number; i++) {
		...
		else if (important_music_number == -9) music_number[i] = -1; // --가 아닐경우 music_number 배열에 -1 표시
