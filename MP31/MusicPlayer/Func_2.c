...
void Musiclist_ContinuePlay_2() {
	...
	//address_comparison = strcmp(fileread, before_fileread); //fileread랑 before_fileread(맨 처음 읽은 링크)와 비교를 해 같은지, 아닌지에 대한 값을 address_comparison에 저장

	/*
	if (address_comparison != 0) { //.txt파일 첫 줄로 초기화되지 않았을때. (fileread와 before_fileread가 같지 않을때)
		while (1) {
			fgets(fileread, sizeof(fileread), fp); 
			address_comparison = strcmp(fileread, before_fileread);

			if (address_comparison == 0) break; //.txt파일 첫 줄이면 종료
		}
	}
	*/
